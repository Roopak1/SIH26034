...
import { Check, X, HelpCircle, Package, Building2, Calendar, Phone, DollarSign, Globe, Scale, SearchCheck } from 'lucide-react';
...
export default function ExtractedInfoTable({ product }) {
  ...
  const sections = [
    ...
    {
      title: 'Public Reference Cross-Check',
      icon: SearchCheck,
      items: [
        { label: 'Identity Status', value: product.web_verification?.identity_status || 'UNVERIFIED' },
        { label: 'Image Surface', value: product.web_verification?.image_view || 'UNKNOWN' },
        {
          label: 'Reference Confidence',
          value: product.web_verification?.confidence !== undefined
            ? `${Math.round(Number(product.web_verification.confidence) * (Number(product.web_verification.confidence) <= 1 ? 100 : 1))}%`
            : null,
        },
        { label: 'Search Queries', value: product.web_verification?.searched_queries?.join(' • ') || null },
        { label: 'Cross-Check Summary', value: product.web_verification?.summary || null },
      ],
    },
  ];
