...
DEMO_SAMPLES = {
    "sample_compliant": ProductData(
        ...
        web_verification=WebVerification(
            ...
            summary="Demo sample; public identity verification is intentionally not run.",
        ),
        raw_evidence=[
...
class ComplianceService:
    ...
    def process_inspection(
        ...
        # 2. Information Extraction (explicit demo route or live Gemini AI)
        if force_demo_sample and force_demo_sample in DEMO_SAMPLES:
            ...
            # Live uploads must fail closed. Demo fallback is available only
            # through the explicit /demo-analyze route, otherwise a bad or
            # missing key could make an unrelated product look compliant.
            raise RuntimeError(
                "Gemini AI is not configured for live inspection. "
                "Add GEMINI_API_KEY to backend/.env or use a demo sample."
            )
        else:
            # AI extracts and cross-checks; the deterministic rule engine still
            # makes the legal/compliance decision. Never replace a failed live
            # extraction with a compliant demo product.
        ...
        response = InspectionResponse(
            ...
            is_demo=is_demo,
            ai_model=(
                ai_service.last_call_metadata.get("model")
                if not is_demo
                else None
            ),
        )
