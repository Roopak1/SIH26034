"""Deterministic compliance rules for LexScan QA packaged commodities.
rules (QA-001 through QA-010).
...
def check_lm_001_manufacturer(product: ProductData) -> RuleResult:
    """QA-001: Manufacturer / Packer / Importer Name & Address Declaration.
...
def check_lm_002_country_of_origin(product: ProductData) -> RuleResult:
    """QA-002: Country of Origin Declaration.
...
def check_lm_003_generic_name(product: ProductData) -> RuleResult:
    """QA-003: Generic / Common Product Name."""
...
def check_lm_004_net_quantity(product: ProductData) -> RuleResult:
    """QA-004: Net Quantity Declaration."""
...
def check_lm_005_mfg_packing_date(product: ProductData) -> RuleResult:
    """QA-005: Date of Manufacture or Packing."""
...
def check_lm_006_best_before_use_by(product: ProductData) -> RuleResult:
    """QA-006: Best Before / Use By Date with actual expiry validation."""
...
def check_lm_008_mrp_tax_inclusive(product: ProductData) -> RuleResult:
    """QA-008: MRP Tax-Inclusive Indication."""
...
def check_lm_009_consumer_care(product: ProductData) -> RuleResult:
    """QA-009: Consumer Care / Grievance Redressal Contact.
...
def check_qa_010_public_identity(product: ProductData) -> RuleResult:
    """QA-010: Conservative public-reference identity cross-check.

    This rule never certifies authenticity. It only converts the AI's
    grounded comparison into a fail-closed compliance signal:
    - MISMATCH => FAIL
    - MATCH with at least one source => PASS (identity consistency only)
    - no reliable comparison => REVIEW
    - demo data => N/A
    """

    verification = product.web_verification
    status = (verification.identity_status or "UNVERIFIED").strip().upper()
    sources = verification.sources or []
    source_text = "; ".join(
        f"{source.title or 'Public source'}: {source.url}"
        for source in sources
        if source.url
    )
    evidence = source_text or "No public verification source recorded."

    if status == "MISMATCH":
        return RuleResult(
            rule_id="QA-010",
            rule_name="Public Product Identity Cross-Check",
            field="web_verification.identity_status",
            status=ComplianceStatus.FAIL,
            detected_value="MISMATCH DETECTED",
            evidence=evidence,
            reason=(
                "The visible product identity or package design conflicts "
                "with the strongest public reference found by the grounded "
                "cross-check. This is a serious authenticity/mislabel risk; "
                "it is not by itself a legal finding that the item is counterfeit."
            ),
            recommendation=(
                "Hold the item for officer review. Compare the physical pack "
                "with the official manufacturer reference and verify batch, "
                "seal, barcode, and purchase records."
            ),
        )

    if status == "MATCH" and sources:
        return RuleResult(
            rule_id="QA-010",
            rule_name="Public Product Identity Cross-Check",
            field="web_verification.identity_status",
            status=ComplianceStatus.PASS,
            detected_value="IDENTITY CONSISTENT WITH PUBLIC REFERENCE",
            evidence=evidence,
            reason=(
                "Visible identity is consistent with a grounded public "
                "reference. This does not certify that the physical item is genuine."
            ),
            recommendation=(
                "Continue checking the complete physical package, including "
                "batch, seal, date, MRP, and hidden panels."
            ),
        )
    ...
    if status == "NOT_APPLICABLE":
        return RuleResult(
            ...
            field="web_verification.identity_status",
            status=ComplianceStatus.NA,
            detected_value="Not run for demo data",
            evidence="Demo sample; no public identity check performed.",
            reason="Demo data is intentionally evaluated without web identity verification.",
            recommendation=None,
        )
    ...
    return RuleResult(
        ...
        field="web_verification.identity_status",
        status=ComplianceStatus.REVIEW,
        detected_value="IDENTITY UNVERIFIED",
        evidence=evidence,
        reason=(
            "No reliable public reference match was recorded, or the image "
            "does not show enough of the package for a defensible comparison. "
            "The system must not call the product real or authentic."
        ),
        recommendation=(
            "Upload clear front and back images and verify the brand/product "
            "against an official manufacturer source."
        ),
    )
