...
from app.rules.common_rules import check_qa_010_public_identity
...
def test_rule_engine_compliant_product():
    product = ProductData(
        ...
        web_verification=WebVerification(
            ...
            sources=[{"title": "Official product reference", "url": "https://example.com/product"}],
        ),
    )
...
# ============================================================
# QA-010 Public identity cross-check
# ============================================================

def test_qa010_public_identity_mismatch_fails_closed():
    product = ProductData(
        web_verification=WebVerification(
            identity_status="MISMATCH",
            sources=[{"title": "Official reference", "url": "https://example.com/product"}],
        )
    )

    result = check_qa_010_public_identity(product)

    assert result.rule_id == "QA-010"
    assert result.status == ComplianceStatus.FAIL


def test_qa010_public_identity_match_is_only_consistency_pass():
    product = ProductData(
        web_verification=WebVerification(
            identity_status="MATCH",
            sources=[{"title": "Official reference", "url": "https://example.com/product"}],
    ...
    assert result.status == ComplianceStatus.PASS
    assert "not certify" in result.reason


def test_qa010_public_identity_unverified_requires_review():
    result = check_qa_010_public_identity(ProductData())

    assert result.status == ComplianceStatus.REVIEW
    assert "must not call" in result.reason
